...
```
`curl --request GET \ --url https://api.vogent.ai/api/agents/{agentId}/versioned_prompts/{id} \ --header 'Authorization: Bearer <token>'`
```
...
[List versioned prompts](/api-reference/list-versioned-prompts)[Update versioned prompt](/api-reference/update-versioned-prompt)